...
template <Scalar, size_t rank>
using GriddedCoeffTensor = eigen::Tensor<Scalar>,
      rank > ;
...
class ScatteringToSpectralTransformer {
  ...
  eigen::VectorMap<std::complex<Scalar>> get_output_sequence(
      size_t major_index, SpectralCoeffTensor<Scalar, rank - 1> &out) {
...
class ScatteringData<DataFormat::Gridded, DataFormat::Gridded> {
 ...
 protected:
  eigen::Tensor<CoeffType, 7>
      phase_matrix_data;  // f x T x elements x (inc. angles) x (scat. angles)
  eigen::Tensor<CoeffType, 7>
      extinction_matrix_data;  // f x T x elements x (inc. angles) x (scat.
                               // angles)
  eigen::Tensor<CoeffType, 7>
      absorption_vector_data;  // f x T x elements x (inc. angles) x (scat.
                               // angles)
  eigen::Tensor<CoeffType, 6>
      backscattering_coefficient;  // f x T x elements x (inc. angles) x (scat.
                                   // angles)
  eigen::Tensor<CoeffType, 6>
      forwardscattering_coefficient;  // f x T x elements x (inc. angles) x
                                      // (scat. angles)
};
